...
```lua
COMPONENTS.Callbacks:RegisterServerCallback('Admin:MyAction', function(source, data, cb)
...
```lua
COMPONENTS.Callbacks:RegisterServerCallback('Admin:GetPlayerInfo', function(source, data, cb)
    local player = Fetch:Source(source)
    ...
    if player.Permissions:IsAdmin() then
        ...
        cb({
            ...
            job = COMPONENTS.Jobs:GetJob(char:GetData('SID')),
            inventory = COMPONENTS.Inventory:Get(char:GetData('SID')),  -- Admin-only
            bankBalance = COMPONENTS.Banking.Balance:Get(account)        -- Admin-only
        })
...
COMPONENTS.Chat:RegisterAdminCommand('mycommand', function(source, args, rawCommand)
    -- Only runs if player is admin
end, { help = 'Description' })

-- Automatically checks IsStaff()
COMPONENTS.Chat:RegisterStaffCommand('mycommand', function(source, args, rawCommand)
...
AddEventHandler('myResource:server:AdminAction', function(data)
    ...
    if not player or not player.Permissions:IsAdmin() then
        COMPONENTS.Logger:Warn('Security', 'Unauthorized admin action attempt', {
            console = true,
            file = true,
            discord = true
        }, {
...
```lua
COMPONENTS.Logger:Warn('Admin', string.format(
...
<AccordionGroup>
  ...
  <Accordion title="Log All Destructive Actions" icon="file-lines">
    ...
    COMPONENTS.Logger:Warn('Admin', actionDescription, {
        console = true,
        file = true,
        database = true,
        discord = true
