...
```lua
-- Client side
local currentAction = Progress:CurrentAction()
...
AddEventHandler('Inventory:Client:ItemRemoved', function(item)
    local action = Progress:CurrentAction()
...
if Progress:Modifier(150, 300000) then
...
if Progress:Modifier(50, 30000) then
    print('Slowed!')
end
```
...
AddEventHandler('crafting:client:CraftItem', function(recipe)
    local hasItems = Inventory.Check.Player:HasItems(recipe.materials)

    if not hasItems then
        Notification:Error('Missing materials')
        return
    end
...
AddEventHandler('lockpick:client:Start', function(doorId)
    if not Inventory.Check.Player:HasItem('lockpick', 1) then
        Notification:Error('Need a lockpick')
        return
    end
    ...
    Progress:ProgressWithTickEvent({
        ...
        if math.random(100) <= 10 then
            Notification:Warn('Lockpick slipped!')
        ...
        if not cancelled then
            ...
            if successChance <= 60 then
                ...
                Notification:Success('Door unlocked')
            else
                TriggerServerEvent('lockpick:server:Break')
                Notification:Error('Lockpick failed')
            end
        end
    end)
end)
...
AddEventHandler('ems:client:Revive', function(targetId)
    ...
    if distance > 3.0 then
        Notification:Error('Too far away')
        return
    end
    ...
    Progress:Progress({
        ...
        if not cancelled then
            ...
            if newDist <= 3.0 then
                ...
                Notification:Error('Patient moved too far')
            end
        end
    end)
end)
...
AddEventHandler('crafting:server:Complete', function(recipeId)
    ...
    for _, mat in ipairs(recipe.materials) do
        if not Inventory.Items:Has(stateId, 1, mat.item, mat.count) then
            hasMaterials = false
            break
        end
    end
    ...
    if hasMaterials then
        -- Remove materials
        for _, mat in ipairs(recipe.materials) do
            Inventory.Items:Remove(stateId, 1, mat.item, mat.count)
        end
...
Progress:Progress({
    ...
    if not cancelled then
        ...
        if distance > 5.0 then
            Notification:Error('You moved too far')
            return
        end
