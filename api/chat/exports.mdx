...
Access via `COMPONENTS.Chat` (server-side only).
...
```lua
COMPONENTS.Chat:RegisterCommand(command, callback, suggestion, arguments, job)
```
...
```lua
-- Simple command
COMPONENTS.Chat:RegisterCommand('me', function(source, args, rawCommand)
    ...
    COMPONENTS.Chat.Send.System:Single(source, name .. ' ' .. message)
end, {
...
COMPONENTS.Chat:RegisterCommand('cuff', function(source, args, rawCommand)
...
```lua
COMPONENTS.Chat:RegisterAdminCommand(command, callback, suggestion, arguments)
```
...
```lua
COMPONENTS.Chat:RegisterAdminCommand('heal', function(source, args, rawCommand)
    local targetId = tonumber(args[1]) or source
    COMPONENTS.Callbacks:ClientCallback(targetId, 'Damage:Heal', true)

    COMPONENTS.Chat.Send.System:Single(source, 'Player healed')
end, {
    help = 'Heal a player',
    params = {
...
```lua
COMPONENTS.Chat:RegisterStaffCommand(command, callback, suggestion, arguments)
```
...
```lua
COMPONENTS.Chat.Send.Server:All(message)
```
...
```lua
COMPONENTS.Chat.Send.Server:All('Server restart in 5 minutes')
```
...
```lua
COMPONENTS.Chat.Send.Server:Single(source, message)
```
...
```lua
COMPONENTS.Chat.Send.System:All(message)
```
...
```lua
COMPONENTS.Chat.Send.System:Single(source, message)
```
...
COMPONENTS.Chat.Send.System:Single(source, 'Your request has been submitted')
```
...
```lua
COMPONENTS.Chat.Send.System:Broadcast(message)
```
...
```lua
COMPONENTS.Chat.Send.Broadcast:All(author, message)
```
...
```lua
COMPONENTS.Chat.Send.OOC(source, message)
```
...
```lua
COMPONENTS.Chat.Send.Services:Emergency(source, message)
```
...
```lua
-- Player calls 911
COMPONENTS.Chat.Send.Services:Emergency(source, 'Shots fired at Legion Square, multiple victims')
```
...
```lua
COMPONENTS.Chat.Send.Services:EmergencyAnonymous(source, message)
```
...
```lua
COMPONENTS.Chat.Send.Services:EmergencyRespond(source, target, message)
```
...
```lua
COMPONENTS.Chat.Send.Services:NonEmergency(source, message)
```
...
```lua
COMPONENTS.Chat.Send.Services:NonEmergencyAnonymous(source, message)
```
...
```lua
COMPONENTS.Chat.Send.Services:NonEmergencyRespond(source, target, message)
```
...
```lua
COMPONENTS.Chat.Send.Services:Dispatch(source, message)
```
...
```lua
COMPONENTS.Chat.Send.Services:TestResult(source, message)
```
...
<AccordionGroup>
  ...
  <Accordion title="Validate Command Arguments" icon="shield-check">
    ```lua
    COMPONENTS.Chat:RegisterCommand('give', function(source, args, rawCommand)
        local targetId = tonumber(args[1])
        local item = args[2]
        local count = tonumber(args[3]) or 1

        if not targetId or not item then
            COMPONENTS.Chat.Send.System:Single(source, 'Usage: /give [id] [item] [count]')
            return
        end
