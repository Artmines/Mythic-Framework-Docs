...
Access via `COMPONENTS.Loans` (server-side only).
...
```lua
COMPONENTS.Loans:GetAllowedLoanAmount(stateId, type)
```
...
```lua
local loanInfo = COMPONENTS.Loans:GetAllowedLoanAmount(char:GetData('SID'), 'vehicle')
...
```lua
COMPONENTS.Loans:GetPlayerLoans(stateId, type)
```
...
```lua
COMPONENTS.Loans:CreateVehicleLoan(targetSource, VIN, totalCost, downPayment, totalWeeks)
```
...
local success = COMPONENTS.Loans:CreateVehicleLoan(source, vehicleVIN, 85000, 20000, 12)
...
```lua
COMPONENTS.Loans:CreatePropertyLoan(targetSource, propertyId, totalCost, downPayment, totalWeeks)
```
...
```lua
COMPONENTS.Loans:MakePayment(source, loanId, inAdvanced, advancedPaymentCount)
```
...
```lua
local result = COMPONENTS.Loans:MakePayment(source, loanId, false)
...
```lua
COMPONENTS.Loans:HasRemainingPayments(assetType, assetId)
```
...
```lua
COMPONENTS.Loans:GetDefaultInterestRate()
```
...
```lua
COMPONENTS.Loans.Credit:Get(stateId)
```
...
```lua
COMPONENTS.Loans.Credit:Set(stateId, newVal)
```
...
```lua
COMPONENTS.Loans.Credit:Increase(stateId, increase)
```
...
```lua
COMPONENTS.Loans.Credit:Decrease(stateId, decrease)
```
