...
<Note>
Discord logging only fires when `GlobalState.IsProduction` is true AND the log level meets the minimum threshold (`COMPONENTS.Convar.LOGGING.value`). It will not send webhooks in development mode.
</Note>
...
1. `COMPONENTS.Proxy.DatabaseReady` is true (database connection established)
