...
exports['mythic-base']:ExtendComponent('Inventory', {
    AddItem = function(self, characterId, item, count, metadata)
        -- Log before
        COMPONENTS.Logger:Trace('Inventory', 'Adding item', {
            console = true
        }, {
            character = characterId,
            item = item,
...
AddEventHandler('Core:Shared:Ready', function()
    ...
    RegisterNetEvent('jobs:server:clockIn', function(jobId)
        ...
        local char = player:GetData('Character')
        if char then
            COMPONENTS.Jobs:ClockIn(char:GetData('SID'), jobId)
        end
    end)
end)
```
...
COMPONENTS.Database.Game:findOne({ collection = 'characters', query = {} }, callback)
```
