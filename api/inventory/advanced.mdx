...
**Syntax:**
```lua
Inventory.Stash:Open(source, invType, identifier)
```
...
RegisterServerEvent('property:server:OpenStash', function()
    ...
    if propertyId then
        -- Open property stash
        Inventory.Stash:Open(src, 13, propertyId)
    else
...
RegisterServerEvent('gang:server:OpenStash', function(gangId)
    ...
    if char:GetData('GangChain') == gangId then
        -- Open gang stash
        Inventory.Stash:Open(src, 13, string.format("gang_%s", gangId))
    else
        TriggerClientEvent('mythic-notifications:client:Send', src, {
...
RegisterServerEvent('cache:server:Open', function(cacheId)
    ...
    -- Open cache
    Inventory.Stash:Open(src, 13, string.format("cache_%s", cacheId))
end)
```
...
**Syntax:**
```lua
Inventory.Shop:Open(source, identifier)
```
...
RegisterServerEvent('shop:server:Open247', function(shopId)
    ...
    -- Open shop
    Inventory.Shop:Open(src, shopId)
end)
...
RegisterServerEvent('shop:server:OpenPoliceArmory', function()
    ...
    -- Check if police
    if Jobs.Permissions:HasJob(src, 'police') then
        Inventory.Shop:Open(src, 'police_armory')
    else
        TriggerClientEvent('mythic-notifications:client:Send', src, {
            message = 'Police only',
            type = 'error'
...
**Syntax:**
```lua
Inventory.Search:Character(source, targetSource, characterId)
```
...
RegisterServerEvent('police:server:SearchPlayer', function(targetSource)
    ...
    Inventory.Search:Character(src, targetSource, targetChar:GetData('SID'))
...
**Syntax:**
```lua
Inventory:Rob(source, targetSource, characterId)
```
...
RegisterServerEvent('crime:server:RobPlayer', function(targetSource)
    ...
    -- Rob player
    Inventory:Rob(src, targetSource, targetChar:GetData('SID'))
...
**Syntax:**
```lua
Inventory:CreateDropzone(routeId, coords)
```
...
local function CreateDrugRoute(routeId)
    ...
    for i, coords in ipairs(dropPoints) do
        local dzId = Inventory:CreateDropzone(routeId, coords)
...
**Syntax:**
```lua
Inventory:CheckDropZones(routeId, coords)
```
...
RegisterServerEvent('drugs:server:CheckDrop', function()
    ...
    local dropzone = Inventory:CheckDropZones(src, coords)
...
**Syntax:**
```lua
Inventory:RemoveDropzone(routeId, dropzoneId)
```
...
RegisterServerEvent('drugs:server:CompleteDelivery', function(dropzoneId)
    ...
    if dropzone and dropzone.id == dropzoneId then
        -- Remove dropzone
        Inventory:RemoveDropzone(src, dropzoneId)
        ...
        Notification:Success(src, 'Delivery complete +$500')
    end
end)
```
...
**Syntax:**
```lua
Inventory:DropExists(routeId, dropzoneId)
```
...
RegisterServerEvent('property:server:OpenStorage', function()
    ...
    if property.owner == char:GetData('SID') or Properties:HasKeys(src, propertyId) then
        -- Open property stash
        Inventory.Stash:Open(src, 13, string.format("property_%s", propertyId))
    else
        TriggerClientEvent('mythic-notifications:client:Send', src, {
...
RegisterServerEvent('drugs:server:StartDelivery', function()
    ...
    -- Check has drugs
    if not Inventory.Items:Has(char:GetData('SID'), 1, 'coke_brick', 1) then
        TriggerClientEvent('mythic-notifications:client:Send', src, {
...
RegisterServerEvent('drugs:server:CompleteDrop', function(dropzoneId)
    ...
    for i, dz in ipairs(ActiveRoutes[src].dropzones) do
        if dz.id == dropzoneId and not dz.completed then
            ...
            local currentCash = char:GetData('Cash')
            char:SetData('Cash', currentCash + dz.reward)

            Notification:Success(src, string.format('Drop complete +$%d (%d/%d)',
                dz.reward,
            ...
            if ActiveRoutes[src].completed >= ActiveRoutes[src].total then
                ...
                local bonusCash = char:GetData('Cash')
                char:SetData('Cash', bonusCash + 1000)

                Notification:Success(src, 'All deliveries complete! +$1000 bonus')
