...
**Syntax:**
```lua
Crafting:RegisterBench(id, label, targeting, location, restrictions, recipes, canUseSchematics)
```
...
**Syntax:**
```lua
Crafting:AddRecipeToBench(benchId, recipeId, recipe)
```
...
**Syntax:**
```lua
Crafting.Craft:Start(crafter, bench, result, qty)
```
...
RegisterServerEvent('crafting:server:StartCraft', function(benchId, recipeId, quantity)
    ...
    -- Start crafting
    local result = Crafting.Craft:Start(stateId, benchId, recipeId, quantity)
...
AddEventHandler('crafting:client:OpenBench', function(data)
    ...
    Callbacks:ServerCallback('crafting:server:GetBench', benchId, function(benchData)
        if benchData then
            -- Open crafting UI
            SendNUIMessage({
                type = 'OPEN_CRAFTING',
            ...
            Notification:Error('Cannot access this bench')
        end
    end)
end)
