...
COMPONENTS.Database:insertOne('logs', { action = 'login', player = source })
...
exports['mythic-base']:RegisterComponent('MyFeature', {
    ...
    GetData = function(self, id)
        return COMPONENTS.Database:findOne('myfeature', { _id = id })
    end
})
```
...
<AccordionGroup>
  ...
  <Accordion title="5. Testability" icon="vial">
    ...
    exports['mythic-base']:RegisterComponent('Calculator', {
        ...
        SaveResult = function(self, result, database)
            database = database or COMPONENTS.Database
            return database:insert('calculations', { result = result })
        end
    })
    ```
  </Accordion>
