...
<AccordionGroup>
  <Accordion title="Invalid License Key" icon="key">
    ...
    **Solutions:**
    ...
    3. Generate new key if IP changed: [keymaster.fivem.net](https://keymaster.fivem.net)
...
<AccordionGroup>
  <Accordion title="Server Lag / High CPU" icon="gauge-high">
    ...
    **Diagnosis:**
    ...
    # Check which resources using most CPU
    ```

    **Solutions:**
    1. **Optimize problem resources:**
       - Restart high-CPU resources
       - Check for infinite loops in custom scripts
       - Update to latest versions

    2. **Reduce player count:**
       - Lower `sv_maxclients` if server can't handle load

    3. **Database optimization:**
       ```javascript
       # MongoDB - Add indexes
       db.characters.createIndex({ "owner": 1 })
       db.inventory.createIndex({ "owner": 1, "item_id": 1 })
       ```
  ...
  <Accordion title="Memory Leak" icon="memory">
    **Symptoms:**
    - Server RAM usage constantly increasing
    - Server crashes after hours of uptime
    - `Out of memory` errors

    **Solutions:**
    1. **Monitor with resmon:**
       Track which resources grow over time

    2. **Schedule restarts:**
       ```bash
       # Linux cron - restart daily at 4 AM
       0 4 * * * systemctl restart mythic-fivem
       ```

    3. **Check for leaks in custom resources:**
       - Remove event listeners properly
       - Clear timers and intervals
       - Avoid global table growth

    4. **Update resources:**
       - Ensure all resources are latest versions
       - Check for known memory leak fixes
  </Accordion>

  <Accordion title="Database Performance" icon="database">
    **Symptoms:**
    - Slow inventory loading
    - Character selection lag
    - Transaction delays
